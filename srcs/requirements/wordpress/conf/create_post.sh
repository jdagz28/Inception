...
POST1_THUMBNAIL_URL="https://butedemo.wordpress.com/wp-content/uploads/2023/07/02d34-sea-path-ocean-dock-wave-lake-1387589-pxhere.com_.jpg?w=1024"
...
POST2_THUMBNAIL_URL="https://www.meme-arsenal.com/memes/7a6bbb1ef560defa6f02c31e3ae20a92.jpg"
...
create_post() {
    ...
    THUMBNAIL_ID=$(wp media import "$thumbnail_url" --path="$WP_PATH" --porcelain --allow-root)
    wp post meta set "$POST_ID" _thumbnail_id "$THUMBNAIL_ID" --path="$WP_PATH" --allow-root
}

create_post "$POST1_AUTHOR" "$POST1_TITLE" "$POST1_CONTENT" "$POST1_THUMBNAIL_URL"
create_post "$POST2_AUTHOR" "$POST2_TITLE" "$POST2_CONTENT" "$POST2_THUMBNAIL_URL"
